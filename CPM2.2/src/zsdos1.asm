;* $Id: zsdos1.asm 834 2006-09-14 07:43:55Z Hharte $               *
...
ROMMED	EQU	FALSE		; Separate data and code?
...
PUBLIC ZSDOS
ZSDOS	EQU	BDOS_LOC ;$		; Start address ZSDOS
...
LISTO	EQU	BIOS+000FH	; List Output
...
;   ID string added for easy identification in running system (hfb)
	  ...
	  IF  ZS
	DB	'ZSDOS '	; Used in CP/M for serial number. these bytes
	  ELSE			; are patched by INSTALOS to contain the serial
	DB	'ZDDOS '	; Number of the running system so MOVCPM can
...
STBDSC:	DW	ERRORS		; Bad sector message
STSEL:	DW	ERRORS		; Select error
STRO:	DW	ERRORS		; Drive read only
SFILRO:	DW	ERRORS		; File read only

; External Path Name

PATH:	DW	PATHAD		; Path address for file open, 0 if no path

; Wheel Byte Pointer

WHEEL:	DW	WHLADR		; Address of wheel byte, 0 if none

; User configuration byte

FLAGS:	DB	FLGBITS		; Flag byte set in zsdos.lib
...
GSTIME:	DW	DOTDER		; Address of get/set time/date routine (hfb)
	  IF	ZS
STLAV:	DW	DOTDER		; Address of stamp last access routine
STCRV:	DW	DOTDER		; Address of stamp create routine
STUPDV:	DW	DOTDER		; Address of stamp modify routine
	  ELSE
STLAV:	DW	STIME		; Address of stamp last access routine
STCRV:	DW	STIME		; Address of stamp create routine
STUPDV:	DW	STIME		; Address of stamp modify routine
	  ENDIF
GETSTV:	DW	DOTDER		; Address of get stamp routine
PUTSTV:	DW	DOTDER		; Address of set stamp routine
	DW	DOTDER		; Dummy vector to disable with ZSCONFIG
UNLOAD:	DW	0		; Pointer to remove Time Stamp routine
...
FCONTP:	DB	0		; List enable flag (Control-P) - used by BGii
LASTCH:	DB	0		; Last character - used by BGii

;--------------------------------------------------------------------
; The following locations MUST remain in EXACTLY this order

USER:	DB	0		; User number - used by BGii
DEFDRV:	DB	0		; Default drive number - used by BGii and DS
...
BGHIRAM:
DMA:	DW	0080H		; DMA address

TRANS:	DW	0		; Translation vector
TEMP0:	DW	0		; Number of files on drive


DIRBUF:	DW	0		; Directory buffer pointer - used by bgii
...
DPBOF	EQU	$-ZSDOS ;MAXSEC-ZSDOS ;$-ZSDOS		; Value needed by ZSDOS

MAXSEC:	DW	0		; Number of sectors/track
NBLOCK:	DB	0		; Block shift
...
ERMODE:	DB	0		; BDOS error mode

ARWORD:	DW	0		; De argument on entry - used for BGii
DEVAL:	DW	0		; Return value for DE reg
SPSAVE:	DW	0		; Stack pointer location
	...
	DB	' C.W.Cotrill & H'
	DB	'.F.Bow'
IXSAVE:	DB	'er'		; User's IX register
ZSDOSS:				; ZSDOS stack
...
CTABLE:
	  IF  ROMMED
	DW	RAMINI		; Set up RAM
	  ELSE
	DW	ERROR5		; Warm boot (BIOS) with ERMODE clear
	  ENDIF
	DW	CMND01		; Console input
	DW	WRCON		; Console output
	DW	READER		; Reader input (BIOS)
	DW	PUNCH		; Punch output (BIOS)
	DW	LISTO		; List output (BIOS)
	DW	CMND06		; Direct console I/O
	DW	CMND07		; Get I/O byte
	DW	CMND08		; Set I/O byte
	DW	CMND09		; Print string
	DW	CMND10		; Read console buffer
	DW	CMND11		; Get console status
	...
	DW	DUMMY		; Function 38 (unused)
	DW	CMND39		; Return fixed disk login vector
	DW	CMND40		; Write random with zero fill
	DW	DUMMY		; Function 41 (unused)
	DW	DUMMY		; Function 42 (unused)
	DW	DUMMY		; Function 43 (unused)
	DW	DUMMY		; Function 44 (unused)
	DW	CMND45		; Set Error Mode
	DW	DUMMY		; Function 46 (unused)
	DW	CMND47		; Return DMA
	DW	CMND48		; Return DOS version

MAXCMD	EQU	($-CTABLE)/2	; Jww

	DW	CMD98		; Get Time	; 49
	DW	CMD99		; Set Time	; 50
	DW	CMD100		; Get Flags	; 51
	DW	CMD101		; Set Flags	; 52
	DW	CMD102		; Get Stamp	; 53
	DW	CMD103		; Put Stamp	; 54
...
RELOG2:
	...
	CALL	SETDSK		; Allow BIOS to detect density change (bh)
	  ELSE
	DB	0,0,0		; Make 3 NOP's to keep constant code (hfb)
	  ENDIF			; (bh)
	POP	AF
...
; DB 0DDH turns the LD HL instructions that follow into LD IX.  In effect,
; this turns the DB 0DDH into a one byte relative jump to SAVHL.  As IX
...
CMND48:
	...
	LD	HL,'D' SHL 8 + VERS ;"D" indicates ZDDOS - ZRDOS returns 0
	  ENDIF
	DB	0DDH		; Trash IX and fall through
...
CMND16:	DB	0CDH	;CALL	SELDR1		; Select drive from FCB
BGPTCH2: DW	SELDR1
...
SEAR12:	LD	A,12
	DB	21H		; Trash HL and fall through
...
CMD98:	LD	C,0		; Set parameter to get time/date
	DB	21H		; ..set for fall thru to GSTD
...
;ERROR	"*** ZSDOS TOO BIG !!!!! ***"
;	      ENDIF		;$-zsdos
;	ORG	BDOS_LOC+0DF1H ;ZSDOS+0DF1H		; Set here for Internal Path
	    ELSE
;	      IF  $-ZSDOS > 0DF9H
;ERROR	"*** ZDDOS TOO BIG !!!!! ***"
;	      ENDIF		;$-zsdos
...
HIRAM:
	  IF  ZS
IPATH:	DB	1,0		; Internal Path = Drive A, User 0
	DW	00,00		; ..two more blank entries
	DB	0		; ...and ending Null
TDFVCT:	DW	00		; Time and date file vector
	  ELSE
TDCHEK:	DB	0		; used by ZDDOS for T&D present flag
	  ENDIF		;Zs
LOGIN:	DW	00		; Login vector
DSKWP:	DW	00		; Disk write protect vector
HDLOG:	DW	00		; Fixed disk login vector
